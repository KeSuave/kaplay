...
import { EdgeMask, type Comp, type CompList, type GameObj } from "../../types";
import { assert, BinaryHeap, type KEventController } from "../../utils";
...
export function level(map: string[], opt: LevelOpt): LevelComp {
  ...
  const getNeighbours = (node: number, diagonals?: boolean) => {
    ...
    const right =
      ...
      costMap![node + 1] !== Infinity;
    const bottom =
      node < numColumns * numRows - numColumns - 1 &&
  ...
  const updateSpatialMap = (level: GameObj<LevelComp>) => {
    ...
    for (const child of level.children) {
      ...
      if (child.tilePos.x != tilePos.x || child.tilePos.y != tilePos.y) {
        ...
        level.removeFromSpatialMap(child);
  ...
  return {
    ...
    raycast(this: GameObj<LevelComp | PosComp>, origin: Vec2, direction: Vec2) {
      ...
      const hit = raycastGrid(
        ...
        (tilePos: Vec2) => {
          ...
          if (minHit) {
            minHit.point = this.fromWorld(minHit.point).scale(invTileWidth);
          }
          return minHit || false;
        },
        64
    ...
    getTilePath(
      ...
      assert(connectivityMap, "connectivityMap is created if not initialized");
    ...
    removeFromSpatialMap(obj: GameObj) {
      const i = tile2Hash(obj.tilePos);
      if (spatialMap![i]) {
    ...
    insertIntoSpatialMap(obj: GameObj) {
      const i = tile2Hash(obj.tilePos);
      if (spatialMap![i]) {
        spatialMap![i].push(obj);
      } else {
    ...
    add(this: GameObj<LevelComp>) {
      ...
      this.onNavigationMapInvalid(() => {
        ...
        this.trigger("navigationMapChanged");
      });
    },
  };
}
